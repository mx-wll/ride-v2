...
import { createRide } from '@/shared/api/rides'
import { useCurrentUserProfile } from '@/features/profile/hooks/use-current-user-profile' // Assuming this hook exists and provides userId
...
export function CreateRideDrawer({ 
  ...
  const handleCreateRide = async () => {
    ...
    const rideDataToInsert = {
        ...
        preset: selectedPreset as 'now' | 'lunch' | 'afternoon' | 'custom',
        distance_km: parseInt(distanceKm, 10) || undefined, // Handle potential NaN
        bike_type: bikeType as 'road' | 'mtb' | 'hybrid' | 'gravel' | 'other',
        starting_point_address: startingPoint, 
        starting_point_coords: currentCoords || undefined 
    };

    // Create the ride using the API layer
    const response = await createRide(rideDataToInsert);

    setIsSubmitting(false)

    if (!response.success) {
      console.error("Error creating ride:", response.error);
      toast.error(`Error: ${response.error?.userMessage || 'Failed to create ride'}`, { id: toastId })
